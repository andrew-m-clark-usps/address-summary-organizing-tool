/**
 * analyzer.js — Data Analysis Functions (ES Module)
 * Ported from the IIFE version in js/analyzer.js
 */
...
export function analyzeQuality(records) {
    ...
    records.forEach(rec => {
        ...
        if (hasState) {
            ...
            if (!VALID_STATES.has(s)) invalidState++;
        }
        ...
        if (hasStreet) {
            const std = standardize(rec.street);
            if (USPS_ABBR_RE.test(std)) standardizedStreetCount++;
        }

        if (rec.street && rec.zip) {
            const key = standardize(rec.street) + '|' + normalizeZip(rec.zip);
...
export function analyzeByState(records) {
    const map = new Map();
    records.forEach(rec => {
        const state = normalizeState(rec.state) || 'UNKNOWN';
...
export function analyzeByCity(records) {
    const map = new Map();
    records.forEach(rec => {
...
export function analyzeByZip(records) {
    const map = new Map();
    records.forEach(rec => {
        const zip = normalizeZip(rec.zip) || 'UNKNOWN';
...
export function summarizeMatches(matchResults, dataA, dataB) {
...
export function geoMatchBreakdown(matchResults) {
    ...
    matchResults.matched.forEach(m => {
        const state = normalizeState(m.recordA.state) || 'UNKNOWN';
...
export function citiesComparison(dataA, dataB) {
    const cityA = new Map();
    const cityB = new Map();
...
export function analyzeMatchDetails(matchResults) {
    ...
    matched.forEach(m => {
        const stateA = normalizeState(m.recordA.state);
        const stateB = normalizeState(m.recordB.state);
        if (stateA && stateB && stateA !== stateB) crossStateMatchCount++;

        const zipA = normalizeZip(m.recordA.zip);
        const zipB = normalizeZip(m.recordB.zip);
        ...
        if (cityA && cityB && stringSimilarity(cityA, cityB) < 1.0) crossCityMatchCount++;
...
export function computeAIMetrics(matchResults, dataA, dataB, qualityA, qualityB, processingMs) {
    const { matched, unmatchedA, unmatchedB } = matchResults;
    const totalA = dataA.length;
    const totalB = dataB.length;
    ...
    const statesAll = new Set([
        ...dataA.map(r => normalizeState(r.state)),
        ...dataB.map(r => normalizeState(r.state))
    ]);
    const vecA = {}, vecB = {};
    statesAll.forEach(s => { vecA[s] = 0; vecB[s] = 0; });
    dataA.forEach(r => { const s = normalizeState(r.state); if (s) vecA[s] = (vecA[s] || 0) + 1; });
    dataB.forEach(r => { const s = normalizeState(r.state); if (s) vecB[s] = (vecB[s] || 0) + 1; });
...
export function runFullAnalysis(dataA, dataB, matchResults, processingMs) {
